...
## Configurar las entidades del Tesla en Home Assistant
Hay que editar y añadir los ficheros `configuration.yaml` y `mqtt_sensors.yaml` que esta en la carpeta `/config` de Home Assistant .
...
En Home Assistant vamos a "Settings" -> "Developer tools" y chequeamos la configuración y reiniciamos. 
Si ahora vamos a "Settings" -> "Devices & services" y vamos a la sección de "Entities" ahora deben aparecer todas las entidades del Tesla.
#### Fichero mqtt home assistant `mqtt_sensors.yaml`
`mqtt_sensors.yaml`:
```
- sensor:
    name: Display Name
    ...
    device: &teslamate_device_info
      ...
      configuration_url: http://192.168.1.20:4000
      manufacturer: Tesla
      model: Model Y
      name: Tesla Model Y
...
```
## Panel de Tesla
Creamos un nuevo panel donde mostraremos las entidades del Tesla, y luego vamos a ella y la editamos en crudo añadiendo este yaml:
#### Yaml vista home assistant
```
views:
  - path: car
    title: Car
                ...
                state:
                  - value: 'off'
                    ...
                    tap_action:
                      ...
                      service: lock.unlock
                      service_data:
                        entity_id: lock.tesla_door_lock
                  - value: 'on'
                    icon: mdi:lock-open
                    color: red
                    tap_action:
